...
controller.hears(['hi'], ['direct_mention', 'direct_message'], function(bot, message) {
  bot.reply(message, 'Hi. Direct message `help` to me to see what I can do.');
});

controller.hears('^help', ['direct_mention', 'direct_message'], function(bot, message) {
  bot.reply(message, "Type `weather` + city name to get current weather conditions " +
            "(for example `weather Boston, MA` or `weather Ottawa, Canada`)");
});

controller.hears('^weather (.*)', ['direct_mention', 'direct_message', 'ambient'], function(bot, message) {
